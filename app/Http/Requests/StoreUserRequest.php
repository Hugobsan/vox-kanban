...
class StoreUserRequest extends FormRequest
{
    ...
    {
        return true; // Permitir cadastro aberto
    }
    ...
    protected function validateRoles($roles, $fail): void
    {
        ...
        // Se não há usuário autenticado (cadastro público)
        if (!$user) {
            // Verificar se todas as roles são 'user'
            $roleNames = \App\Models\Role::whereIn('id', $roles)->pluck('name');
            if ($roleNames->diff(['user'])->isNotEmpty()) {
                $fail('Apenas a role "user" pode ser atribuída em cadastro público.');
            }
            return;
        }
